    ...
    <div class="auto-container">
        ...
        <div class="services-flex-container" id="services-cards">
            ...
            <div class="services-block-five">
                <div class="inner-box">
                    <div class="image">
                        <a href="pro/{{ $pc->id }}"><img style="height:250px" src="{{ asset('userimages/'.$pc->profilePic) }}" alt="" /></a>
                        <a href="pro/{{ $pc->id }}" class="read-more">Read More</a>
                    </div>
                    <div class="lower-content">
                        <div style="border-radius:10px" class="icon"><img src="assets/icon/tap.webp" alt=""></div>
                        <h3><a href="pro/{{ $pc->id }}">{{ $pc->Name }}</a></h3>
                    
                        <div class="text">{{ $pc->Bio }}</div>
                        <div class="text"style=" font-weight:700" >Service charges: <span style="color:red; font-weight:700">Rs500</span> </div>
                        <div style="display:none" class="text">{{ $pc->city }}</div>
                        <div style="display:none" class="text">{{ $pc->state }}</div>
                    </div>
                </div>
                <center> <a style="color: #222222; font-size: 16px;font-weight: 400;padding: 8px 20px;padding-left: 15px; background-color:#fda700" href="pro/{{ $pc->id }}" class="read-more">View Profile</a></center>
            </div>
            @endforeach
        </div>
        <div class="pagination-container">
            {{ $cp->links('pagination::bootstrap-5') }} 
        </div>
    </div>
</section>
