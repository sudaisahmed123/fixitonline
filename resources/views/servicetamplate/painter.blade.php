    ...
    <div class="auto-container">
        ...
        <div class="services-flex-container" id="services-cards">
            ...
            <div class="services-block-five">
                <div class="inner-box">
                    <div class="image">
                        <a href="pro/{{ $ap->id }}"><img style="height:250px" src="{{ asset('userimages/'.$ap->profilePic) }}" alt="" /></a>
                        <a href="pro/{{ $ap->id }}" class="read-more">Read More</a>
                    </div>
                    <div class="lower-content">
                        <div style="border-radius:10px" class="icon"><img src="assets/icon/tap.webp" alt=""></div>
                        <h3><a href="pro/{{ $ap->id }}">{{ $ap->Name }}</a></h3>
                    
                        <div class="text">{{ $ap->Bio }}</div>
                        <div class="text"style=" font-weight:700" >Service charges: <span style="color:red; font-weight:700">Rs500</span> </div>
                        <div style="display:none" class="text">{{ $ap->city }}</div>
                        <div style="display:none" class="text">{{ $ap->state }}</div>
                    </div>
                </div>
                <center> <a style="color: #222222; font-size: 16px;font-weight: 400;padding: 8px 20px;padding-left: 15px; background-color:#fda700" href="pro/{{ $ap->id }}" class="read-more">View Profile</a></center>
            </div>
            @endforeach
        </div>
        <div class="pagination-container">
            {{ $pa->links('pagination::bootstrap-5') }} 
        </div>
    </div>
</section>
