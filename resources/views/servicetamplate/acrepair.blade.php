    ...
    <div class="auto-container">
        ...
        <div class="services-flex-container" id="services-cards">
            ...
            <div class="services-block-five">
                <div class="inner-box">
                    <div class="image">
                        <a href="pro/{{ $ra->id }}"><img style="height:250px" src="{{ asset('userimages/'.$ra->profilePic) }}" alt="" /></a>
                        <a href="pro/{{ $ra->id }}" class="read-more">Read More</a>
                    </div>
                    <div class="lower-content">
                        <div style="border-radius:10px" class="icon"><img src="assets/icon/tap.webp" alt=""></div>
                        <h3><a href="pro/{{ $ra->id }}">{{ $ra->Name }}</a></h3>
                    
                        <div class="text">{{ $ra->Bio }}</div>
                        <div class="text"style=" font-weight:700" >Service charges: <span style="color:red; font-weight:700">Rs500</span> </div>
                        <div style="display:none" class="text">{{ $ra->city }}</div>
                        <div style="display:none" class="text">{{ $ra->state }}</div>
                    </div>
                </div>
                <center> <a style="color: #222222; font-size: 16px;font-weight: 400;padding: 8px 20px;padding-left: 15px; background-color:#fda700" href="pro/{{ $ra->id }}" class="read-more">View Profile</a></center>
            </div>
            @endforeach
        </div>
        <div class="pagination-container">
            {{ $ar->links('pagination::bootstrap-5') }} 
        </div>
    </div>
</section>
